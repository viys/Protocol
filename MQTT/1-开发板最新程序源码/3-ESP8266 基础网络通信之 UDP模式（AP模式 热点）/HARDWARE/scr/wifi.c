...
char UDP_flag = 0;      //0：初始化UDP失败  1：初始化UDP成功
...
{	
	WiFi_RxCounter=0;                               //WiFi接收数据量变量清零                        
	memset(WiFi_RX_BUF,0,WiFi_RXBUFF_SIZE);         //清空WiFi接收缓冲区   
	WiFi_printf("AT+CIPSTART=\"UDP\",\"%s\",%d,%d,0\r\n",ServerIP,ServerPort,local_port);//发送指令
	while(timeout--){                               //等待超时与否
		Delay_Ms(100);                              //延时100ms	
		if(strstr(WiFi_RX_BUF ,"OK"))               //如果接受到OK表示成功
			break;                                  //跳出while循环
		u1_printf("%d ",timeout);                   //串口输出现在的超时时间  
	}
	u1_printf("\r\n");                        //串口输出信息
	if(timeout<=0)return 1;                   //超时错误，返回3
	else                                      //连接成功，准备进入透传
	{
		u1_printf("准备进入透传\r\n");                  //串口显示信息
		WiFi_RxCounter=0;                               //WiFi接收数据量变量清零                        
		...
		if(timeout<=0)return 2;                 //透传超时错误，返回2	
	}
	return 0;	                                //成功返回0	
}
/*-------------------------------------------------*/
...
{	
	...
	u1_printf("准备初始化UDP\r\n");                    //串口提示数据
	res = WiFi_Connect_UDP(100,5050);                  //初始化UDP，100ms超时单位，总计10s超时时间
	if(res==1){                						   //返回1，进入if
		u1_printf("初始化UDP超时，准备重启\r\n");      //串口提示数据
		return 13;                                     //返回13
	}else if(res==2){								   //返回4，进入if，
		u1_printf("进入透传失败\r\n");                 //串口提示数据
		return 14;                                     //返回14
	}	
	u1_printf("初始化UDP成功\r\n");                    //串口提示数据
