...
char Sign[512];                                              //一机一密认证时用的认证信息的缓冲区
int  Sign_len;				                                 //一机一密认证时用的认证信息数据的长度
...
{	
	char temp1[512];                                                       //计算加密的时候，临时使用的缓冲区1
	char temp2[512];                                                       //计算加密的时候，临时使用的缓冲区2
	
	memset(temp1,0,512);                                                                    //临时缓冲区1全部清零
	memset(temp2,0,512);                                                                    //临时缓冲区2全部清零
	memset(Sign,0,512);                                                                     //Sign缓冲区全部清零
	
	memset(ClientID,0,128);                    //客户端ID的缓冲区全部清零
	sprintf(ClientID,"%s",DEVICENAME);         //构建客户端ID，并存入缓冲区
	ClientID_len = strlen(ClientID);           //计算客户端ID的长度
	memset(Username,0,128);                    //客户端ID的缓冲区全部清零
	memset(Passward,0,128);                    //客户端ID的缓冲区全部清零
	
	sprintf(temp1,"clientId%sdeviceName%sproductKey%s",DEVICENAME,DEVICENAME,PRODUCTKEY);                     //构建加密时的明文   
	utils_hmac_md5(temp1,strlen(temp1),temp2,DEVICESECRE,DEVICESECRE_LEN);                                    //以DeviceSecret为秘钥对temp中的明文，进行_hmac_md5加密，结果就是密码，并保存到缓冲区2中
	sprintf(Sign,"productKey=%s&sign=%s&clientId=%s&deviceName=%s",PRODUCTKEY,temp2,DEVICENAME,DEVICENAME);   //构建一机一密认证时用的认证信息
	Sign_len = strlen(Sign);                                                                                  //一机一密认证信息长度
	...
	u1_printf("一机一密认证信息数据：%s\r\n",Sign);                                             //串口输出调试信息
	u1_printf("一机一密认证信息长度：%d\r\n",Sign_len);                                         //串口输出调试信息
}
